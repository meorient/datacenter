package com.meorient.common.dao;
...
public abstract class AbsBaseDao <T extends BasePojo> {
	...
	public List<T> selectList(String mapperId, T t) {
		...
		if (isPaging) {
			PageMethod.startPage(t.getPageNum(), t.getPageSize(), true);
		}
	 ...
	 * @return
	 */
	public Map<String,T> selectMap(T t , String key) {
		Map<String,T> resultMap = template.selectMap(getFullMapperId("selectMap"), t, key);
		return resultMap;
	}
	 ...
	 * @return
	 */
	public Map<String,T> selectMap(String mapperId , T t , String key) {
		Map<String,T> resultMap = template.selectMap(getFullMapperId(mapperId), t , key);
		return resultMap;
	}
